  ...
  <div>
    <div class="toolbar">
      ...
      <div class="toolbar-btn-group">
        <el-button
          type="primary"
          @click="$router.push({ name: 'SolutionInfo' })"
          >添加</el-button
        >
      </div>
    </div>
...
<script>
import { SolutionList, SolutionDelete } from "../../api/solution";
...
export default {
  ...
  methods: {
    ...
    editHandle(row) {
      this.$router.push({ name: "SolutionInfo", query: { id: row.id } });
    },
    deleteHandle(row) {
      this.$confirm("确认是否删除方案数据?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          SolutionDelete(row.id).then((res) => {
            if (res.code === 0) {
              this.$message({
                type: "success",
                message: "删除成功",
